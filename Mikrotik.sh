...
/ip address
add address=192.168.A.1/24 interface=vlan10
add address=192.168.200.1/24 interface=vlan20

# Konfigurasi DHCP Server untuk VLAN 10
/ip pool
add name=dhcp_pool_vlan10 ranges=192.168.A.10-192.168.A.100
/ip dhcp-server
add name=dhcp_vlan10 interface=vlan10 address-pool=dhcp_pool_vlan10
/ip dhcp-server network
add address=192.168.A.0/24 gateway=192.168.A.1 dns-server=8.8.8.8,8.8.4.4

# Konfigurasi DHCP Server untuk VLAN 20
/ip pool
add name=dhcp_pool_vlan20 ranges=192.168.200.10-192.168.200.100
/ip dhcp-server
